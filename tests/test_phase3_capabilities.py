"""Tests for Phase 3: Strategy Engine & Automation - Capabilities Adapter Implementation."""
...
class TestPhase3Capabilities:
    """Test suite for Phase 3 capabilities using the new adapter architecture."""
    ...
    def mock_capabilities_adapter(self):
        """Mock the capabilities adapter for testing."""
        with patch('backend.capabilities') as mock_capabilities:
            # Mock the capabilities adapter
            mock_adapter = Mock()
...
class TestSignalGeneration(TestPhase3Capabilities):
    """Tests for signal generation endpoints."""

    def test_generate_signal_success(self, client, mock_capabilities_adapter):
    ...
    def test_generate_signal_with_custom_types(self, client, mock_capabilities_adapter):
        """Test signal generation with custom signal types."""
        response = client.get("/trade/signal/EURUSD?signal_types=SMA,RSI")

        assert response.status_code == 200
        data = response.json()
        assert data["status"] == "success"
        assert data["signal_types"] == ["SMA", "RSI"]

    def test_generate_signal_not_connected(self, client, mock_capabilities_adapter):
        """Test signal generation when not connected."""
        mock_capabilities_adapter.is_connected = False
        mock_capabilities_adapter.generate_signal.return_value = {"error": "Not connected to Chrome session"}
    ...
    def test_generate_signal_error(self, client, mock_capabilities_adapter):
        """Test signal generation with error."""
        mock_capabilities_adapter.generate_signal.return_value = {"error": "Signal generation failed"}

        response = client.get("/trade/signal/EURUSD")

        assert response.status_code == 500
        data = response.json()
        assert "Signal generation failed" in data["detail"]


class TestAutomatedTrading(TestPhase3Capabilities):
    """Tests for automated trading endpoints."""

    def test_start_automated_trading_success(self, client, mock_capabilities_adapter):
        """Test successful automated trading start."""
    ...
    def test_start_automated_trading_not_connected(self, client, mock_capabilities_adapter):
        """Test automated trading start when not connected."""
        mock_capabilities_adapter.is_connected = False
        mock_capabilities_adapter.manage_automated_trading.return_value = {"error": "Not connected to Chrome session"}

        response = client.post("/auto-trading/start")

        assert response.status_code == 400
        data = response.json()
        assert "Not connected to Chrome session" in data["detail"]

    def test_stop_automated_trading_success(self, client, mock_capabilities_adapter):
        """Test successful automated trading stop."""
        mock_capabilities_adapter.manage_automated_trading.return_value = {
            "status": "stopped",
            "final_stats": {
                "is_running": False,
                "trades_executed": 5,
    ...
    def test_get_automated_trading_status_success(self, client, mock_capabilities_adapter):
        """Test successful automated trading status retrieval."""
        mock_capabilities_adapter.manage_automated_trading.return_value = {
            "status": "running",
            "trading_stats": {
                "is_running": True,
            ...
            "active_trades": [{"asset": "EURUSD", "side": "buy"}]
        }
        ...
        assert data["trading_stats"]["trades_executed"] == 3


class TestStrategyManagement(TestPhase3Capabilities):
    """Tests for strategy management endpoints."""

    def test_list_strategies_success(self, client, mock_capabilities_adapter):
    ...
    def test_get_strategy_success(self, client, mock_capabilities_adapter):
        """Test successful strategy retrieval."""
        mock_capabilities_adapter.manage_strategy.return_value = {
            "strategy": {
                "strategy_id": "quantum_flux_1min",
                "name": "Quantum Flux 1-Minute Strategy",
                "description": "Advanced 1-minute timeframe strategy",
    ...
    def test_get_strategy_not_found(self, client, mock_capabilities_adapter):
        """Test strategy retrieval for non-existent strategy."""
        mock_capabilities_adapter.manage_strategy.return_value = {"error": "Strategy nonexistent not found"}
    ...
    def test_get_strategy_performance_success(self, client, mock_capabilities_adapter):
...
class TestABTesting(TestPhase3Capabilities):
    """Tests for A/B testing endpoints."""

    def test_start_ab_test_success(self, client, mock_capabilities_adapter):
        """Test successful A/B test start."""
        test_data = {
            "test_name": "strategy_comparison_test",
            "strategy_a": "quantum_flux_1min",
            "strategy_b": "conservative_sma",
    ...
    def test_start_ab_test_not_connected(self, client, mock_capabilities_adapter):
        """Test A/B test start when not connected."""
        mock_capabilities_adapter.is_connected = False
        mock_capabilities_adapter.manage_ab_testing.return_value = {"error": "Not connected to Chrome session"}
    ...
    def test_get_ab_test_status_success(self, client, mock_capabilities_adapter):
        """Test successful A/B test status retrieval."""
        mock_capabilities_adapter.manage_ab_testing.return_value = {
    ...
    def test_get_ab_test_results_success(self, client, mock_capabilities_adapter):
    ...
    def test_get_ab_test_results_not_found(self, client, mock_capabilities_adapter):
        """Test A/B test results for non-existent test."""
        mock_capabilities_adapter.manage_ab_testing.return_value = {"error": "Test nonexistent not found or not completed"}

        response = client.get("/tests/ab/results?test_name=nonexistent")
...
class TestPhase3Integration(TestPhase3Capabilities):
    """Integration tests for Phase 3 functionality."""

    def test_full_trading_workflow(self, client, mock_capabilities_adapter):
        """Test a complete trading workflow using Phase 3 features."""
        # 1. Generate signal
    ...
    def test_capabilities_status_includes_phase3(self, client, mock_capabilities_adapter):
