...
class RealtimeDataStreaming(Capability):
    ...
    def __init__(self):
        ...
        self.enable_csv_saving = True  # Flag to enable/disable CSV saving
    ...
    def save_to_csv(self, asset: str, ctx: Ctx) -> None:
        """Save collected candle data to CSV file in the data_stream directory."""
        try:
            ...
            # Create data_stream directory path
            project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
            data_stream_dir = os.path.join(project_root, "Historical_Data", "data_stream")
            
            # Ensure directory exists
            os.makedirs(data_stream_dir, exist_ok=True)
            ...
            filepath = os.path.join(data_stream_dir, filename)
            ...
            if ctx.verbose:
                print(f"💾 [{datetime.now(timezone.utc).strftime('%H:%M:%SZ')}] Saved {len(self.CANDLES[asset])} candles for {asset} to {filename}")
    ...
    def _process_historical_data(self, data: Dict[str, Any], ctx: Ctx) -> None:
        ...
        try:
            if 'history' in data:
                ...
                # Save to CSV if enabled and we have data
                if self.enable_csv_saving and candles:
                    self.save_to_csv(asset, ctx)
